...
using Comment.Infrastructure.Query.Converters;
...
namespace Comment.Infrastructure.Query.Consumers
{
    public class CommentEventConsumer : IEventConsumer
    {
        private readonly ConsumerConfig _config;
        private readonly ICommentEventHandler _commentEventHandler;
        private readonly ILogger<CommentEventConsumer> _logger;
        public CommentEventConsumer(IConfiguration configuration, ICommentEventHandler commentEventHandler,
                ...
                try
                {
                    ...
                    while (true)
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            throw new EventConsumeException(nameof(CommentEventConsumer), $"Error deserializing event: {consumeResult.Message.Value}", ex);
                        }

                        // 1st check if the event is a Comment event
                        var commentHandlerMethod = _commentEventHandler.GetType().GetMethod("OnEvent", new Type[] { @event.GetType() });
                        if (commentHandlerMethod != null)
                        {
                            try
                            {
                                _logger.LogDebug("Invoking {handlerMethod} with {@event}", commentHandlerMethod.Name, @event.ToJson());
                                commentHandlerMethod.Invoke(_commentEventHandler, new object[] { @event });
                            }
                            catch (EventHandlerException ex)
                            {
                                _logger.LogError("Handler method not found {name}", nameof(commentHandlerMethod));
                                throw new EventConsumeException(nameof(CommentEventConsumer), $"Error Invoking {@event.ToJson()}", ex);
                            }
                        ...
                        if (commentHandlerMethod == null)
                        {
                            _logger.LogError("Handler method not found {name}", nameof(CommentEventConsumer));
                            throw new ArgumentNullException(nameof(CommentEventConsumer), "Handler method not found");
                        }
                    }
                }
