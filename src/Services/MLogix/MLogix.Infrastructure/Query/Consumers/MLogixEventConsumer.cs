...
using MLogix.Infrastructure.Query.Converters;
...
namespace MLogix.Infrastructure.Query.Consumers
{
    public class MLogixEventConsumer : IEventConsumer
    {
        private readonly ConsumerConfig _config;
        private readonly IMLogixEventHandler _mLxEventHandler;
        private readonly ILogger<MLogixEventConsumer> _logger;
        public MLogixEventConsumer(IConfiguration configuration, IMLogixEventHandler mLxEventHandler,
                                    ILogger<MLogixEventConsumer> logger)
        {
            ...
            _mLxEventHandler = mLxEventHandler;
            _logger = logger;
        }
            ...
            while (true)
            {
                try
                {
                    ...
                    while (true)
                    {
                        ...
                        {
                            _logger.LogInformation("MLogixEventConsumer: Skipping event {message} as the event was not understood. (Acknowledged)", consumeResult.Message.Value);
                        ...
                        catch (Exception ex)
                        {
                            ...
                            throw new EventConsumeException(nameof(MLogixEventConsumer), $"Error deserializing event: {consumeResult.Message.Value}", ex);
                        }

                        // 1st check if the event is a MLogix event
                        var mLxHandlerMethod = _mLxEventHandler.GetType().GetMethod("OnEvent", new Type[] { @event.GetType() });
                        if (mLxHandlerMethod != null)
                        {
                            try
                            {
                                _logger.LogDebug("Invoking {handlerMethod} with {@event}", mLxHandlerMethod.Name, @event.ToJson());
                                mLxHandlerMethod.Invoke(_mLxEventHandler, new object[] { @event });
                            }
                            catch (EventHandlerException ex)
                            {
                                _logger.LogError("Handler method not found {name}", nameof(mLxHandlerMethod));
                                throw new EventConsumeException(nameof(MLogixEventConsumer), $"Error Invoking {@event.ToJson()}", ex);
                            }
                        ...
                        if (mLxHandlerMethod == null)
                        {
                            _logger.LogError("Handler method not found {name}", nameof(MLogixEventConsumer));
                            throw new ArgumentNullException(nameof(MLogixEventConsumer), "Handler method not found");
                        }
                    }
                }
